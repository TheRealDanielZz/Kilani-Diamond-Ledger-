...
async function run() {
    try {
        ...
        const specDocs = await specsRef.get();
        const currentSpecs = specDocs.docs.map(doc => ({ id: doc.id, ...doc.data() }));

        const specIdMap = new Map();

        for (const item of dataToImport) {
            let spec = currentSpecs.find(s => s.sizeMm === item.size && s.shape === 'Round');
            if (!spec) {
                ...
                await db.collection('specs').doc(newId).set(specData);
        ...
        await db.collection('movements').doc(movId).set(movementData);
