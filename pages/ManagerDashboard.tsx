...
import { Inbox, PackageCheck, Plus, LayoutGrid, List as ListIcon, Image as ImageIcon, AlertOctagon, ChevronRight, Scale, Layers, X } from 'lucide-react';
...
const ManagerDashboard: React.FC<{ currentUser: any }> = ({ currentUser }) => {
  ...
  const handleCount = async () => {
    ...
    if (mixedMode) {
       ...
       if (!mixedNotes) return alert("Please enter a note for mixed return.");
        ...
        if(hasBroken && !brokenReason) return alert("Please enter a reason for the broken stones.");
    ...
    try {
      const itemsToProcess = countTx 
          ? countTx.lines.map(l => ({ specId: l.specId, issuedPcs: l.originalIssuedPcs })) 
          : (isManagerEdit ? editableItems : countBag.items);
          
      const countLines = itemsToProcess.map((item, idx) => ({
          specId: item.specId,
          pcs: counts[idx] ?? (countTx ? countTx.lines[idx].returnedPcs : item.issuedPcs)
      }));
      ...
      if(!mixedMode && Object.values(brokenCounts).some((v: any) => v > 0)) {
         const brokenLines = itemsToProcess.map((item, idx) => ({
             specId: item.specId, 
             pcs: brokenCounts[idx] || 0
         })).filter(l => l.pcs > 0);
         ...
         await store.createInventoryMovement({
            ...
            lines: brokenLines.map(l => {
                ...
                return { specId: l.specId, pcs: l.pcs, ct: l.pcs * (spec?.ctPerStone || 0) };
            }),
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 pb-32">
      ...
      <div className="modular-grid mb-10">
         <div data-tour="manager-requests">
            <Card className="border-zinc-800 p-6 h-full flex flex-col bg-transparent">
               <div className="flex justify-between items-start mb-5">
                  <div className="flex items-center gap-3">
                     <div className="bg-blue-500/10 text-blue-400 p-2.5 rounded-2xl border border-blue-500/20"><Inbox size={20}/></div>
               ...
               <div className="space-y-2 flex-1 overflow-y-auto pr-1">
                  {requests.length === 0 ? <p className="text-zinc-500 text-sm py-4 italic">All requests fulfilled.</p> : 
                     requests.map(r => (
                        <div key={r.id} className="p-3 rounded-xl border border-theme-border flex justify-between items-center bg-black/20">
                           <div>
                              <div className="font-bold text-theme-text-primary">{store.getProject(r.projectId)?.code}</div>
         ...
         <div data-tour="manager-returns">
            <Card className="border-zinc-800 p-6 h-full flex flex-col bg-transparent">
               <div className="flex justify-between items-start mb-5">
                  <div className="flex items-center gap-3">
                     <div className="bg-amber-500/10 text-amber-500 p-2.5 rounded-2xl border border-amber-500/20"><PackageCheck size={20}/></div>
               ...
               <div className="space-y-2 flex-1 overflow-y-auto pr-1">
                  {returnBags.length === 0 ? <p className="text-zinc-500 text-sm py-4 italic">No pending returns.</p> : 
                     returnBags.map((info, idx) => {
                        const {bag: b, tx} = info;
                        return (
                        <div key={tx?.id || b.id || idx} className="p-3 rounded-xl border border-theme-border flex justify-between items-center bg-black/20">
                           <div>
                              <div className="font-bold text-theme-text-primary">Bag #{b.bagNumber} {tx ? '(Partial)' : ''}</div>
         ...
         <div data-tour="gold-price-card" className="md:col-span-2 lg:col-span-1">
            <GoldPriceCard />
         </div>
      </div>
                  ...
                  <section className="space-y-4">
                     ...
                     <div className="bg-theme-input-bg rounded-2xl p-4 border border-theme-border flex flex-wrap gap-2">
                        {SERVICE_OPTIONS.map(svc => (
                           <button key={svc} onClick={() => toggleService(svc)} className={`px-4 py-2 rounded-xl text-sm font-bold border transition-all ${selectedServices.includes(svc) ? 'bg-lux-gold text-black border-lux-gold shadow-sm' : 'bg-theme-modal-bg border-theme-border text-zinc-400 hover:text-theme-text-primary hover:border-zinc-700'}`}>{svc}</button>
                        ))}
                     </div>
                      {selectedServices.includes('Repair') && (
      ...
      {fulfillReq && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
           <Card className="w-full max-w-md p-6">
              ...
              <div className="bg-lux-gold/10 border border-lux-gold/30 p-4 rounded-xl mb-4 text-center">
              ...
              <div className="border border-white/5 rounded-xl overflow-hidden bg-zinc-900/50 mb-6">
                 <div className="hidden sm:grid grid-cols-[1fr_100px] bg-black/30 border-b border-white/5 px-4 py-3 text-[10px] font-black text-zinc-500 uppercase tracking-widest">
                 ...
                 <div className="divide-y divide-white/5 max-h-[250px] overflow-y-auto">
                    {fulfillReq.lines.map((l, i) => {
                       ...
                       return (
                          <div key={i} className="p-3 sm:px-4 sm:py-2">
                             <div className="sm:hidden flex flex-col gap-1">
                                <div className="text-[10px] text-zinc-500 font-bold uppercase tracking-widest">Specification</div>
                                <div className="font-bold text-lux-cream text-sm mb-2">{spec?.label}</div>
                                <div className="grid grid-cols-2">
                                   <div>
                                     <div className="text-[10px] text-zinc-500 font-bold uppercase tracking-widest">Quantity</div>
                                     <div className="text-lux-gold font-mono font-bold">{l.requestedPcs} pcs</div>
                                   </div>
                                </div>
                             </div>
                             <div className="hidden sm:grid grid-cols-[1fr_100px] items-center">
                                <div className="font-bold text-lux-cream text-sm truncate pr-4" title={spec?.label}>{spec?.label}</div>
                                <div className="text-right text-lux-gold font-mono font-bold">{l.requestedPcs} pcs</div>
                             </div>
                          </div>
                       )
                    })}
              ...
              <Input label="Assign Bag Number" value={bagNum} onChange={e => setBagNum(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleFulfill()} autoFocus className="mb-4 font-mono text-xl text-center" />
              <div className="mb-6"><ImageUpload label="Bag Photo" required value={issuedPhoto} onChange={(base64, src) => { setIssuedPhoto(base64); setIssuedPhotoSource(src); }} /></div>
                           ...
                           return (
                              <div key={idx} className="bg-zinc-900 p-3 rounded-xl border border-zinc-800 space-y-2">
                                 ...
                                 <div className="grid gap-2 items-center" style={{gridTemplateColumns:'5fr 2fr 2fr 3fr 3fr'}}>
                                    ...
                                    <div><input type="number" value={returnedVal} onChange={e => setCounts({...counts, [idx]: parseInt(e.target.value) || 0})} className={`w-full bg-black border rounded-lg p-1.5 text-center font-mono text-sm focus:ring-0 ${returnedVal !== (item as any).defaultReturned ? 'border-amber-500 text-amber-400' : 'border-zinc-700 focus:border-lux-gold text-white'}`} /></div>
