  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-8 h-[calc(100vh-60px)] flex flex-col">
      <div className="flex items-center gap-4 mb-6">
         ...
         <div>
            ...
            <div className="flex items-center gap-2 text-xs text-zinc-500">
               <span className="font-mono bg-zinc-900 border border-zinc-800 px-1.5 py-0.5 rounded text-zinc-400">{project.code}</span>
               <span>•</span>
      ...
      <div className="flex-1 flex flex-col md:flex-row gap-8 overflow-hidden">
        ...
        <div className="flex-1 flex flex-col bg-lux-surface rounded-xl border border-lux-border shadow-sm overflow-hidden">
           
           {/* Scrollable Container with Min Width */}
           <div className="flex-1 overflow-auto">
             <div className="min-w-[600px]">
               {/* Header */}
               <div className="grid grid-cols-12 bg-zinc-900 border-b border-lux-border text-xs font-semibold text-zinc-500 uppercase tracking-wide">
                  <div className="col-span-5 p-4">Item Spec</div>
                  <div className="col-span-3 p-4 text-center border-l border-zinc-800">{reportedLabel}</div>
                  <div className="col-span-4 p-4 text-center border-l border-zinc-800 bg-zinc-800/30">Verified Count</div>
               </div>
               ...
               <div>
                 {activeTx?.lines.map((line) => {
                   ...
                   return (
                     <div key={line.specId} className={`grid grid-cols-12 border-b border-zinc-800/50 items-center hover:bg-zinc-900/30 transition-colors ${isDiff ? 'bg-amber-900/10' : ''}`}>
                        ...
                        <div className="col-span-3 p-4 text-center border-l border-zinc-800/50 text-zinc-400 tabular-nums font-mono text-sm">
                           {line.qtyPcs} <span className="text-[10px] text-zinc-600">pcs</span>
                        </div>

                        <div className="col-span-4 p-3 border-l border-zinc-800/50 flex justify-center bg-zinc-900/20">
                           <div className="flex items-center gap-2">
                              <div className="relative">
                                <input 
                                  type="number" 
                                  className={`w-20 text-center font-mono font-medium text-sm rounded-lg border py-1.5 focus:ring-1 focus:ring-offset-1 focus:ring-offset-lux-black transition-all ${isDiff ? 'bg-amber-900/20 border-amber-800 text-amber-400 focus:ring-amber-700' : 'bg-lux-input border-lux-border text-lux-cream focus:ring-lux-gold'}`}
                                  value={count.pcs}
                                  onChange={(e) => {
           ...
           {/* Footer / Summary */}
           <div className="bg-zinc-900 border-t border-lux-border p-4 flex justify-between items-center z-10 relative">
        ...
        <div className="w-full md:w-[300px] space-y-4">
           {hasDiffs && (
             <Card className="p-4 bg-amber-900/10 border-amber-900/30 shadow-none">
                ...
                <textarea 
                  className="w-full text-sm bg-black/40 border-amber-800/50 rounded-lg p-3 text-lux-cream placeholder:text-zinc-600 focus:ring-amber-700 focus:border-amber-600"
                  rows={4}
             ...
             <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between">
                   ...
                   <span className={`font-mono font-bold ${hasDiffs ? 'text-amber-500' : 'text-lux-cream'}`}>
                     {Object.values(counts).reduce((acc, c: any) => acc + c.pcs, 0)}
                   </span>
                </div>
             </div>
           </Card>
