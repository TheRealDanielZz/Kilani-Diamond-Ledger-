...
export const GoldPriceCard: React.FC = () => {
    ...
    return (
        <Card className={`p-5 md:p-4 lg:p-5 h-full flex flex-col justify-between relative overflow-hidden group bg-gradient-to-br from-[#1F2128] to-black ${hasError ? 'border-red-500/30' : 'border-lux-gold/20'}`}>
            {/* Background Glow */}
            ...
            <div className="flex justify-between items-start mb-2 relative z-10">
                <div className="flex items-center gap-2">
                    <div className={`p-2 rounded-xl transition-colors duration-500 ${hasError ? 'bg-red-500/10 text-red-500' : isLive ? 'bg-lux-gold/10 text-lux-gold' : 'bg-blue-500/10 text-blue-400'}`}>
                ...
                <div className="flex gap-1">
                   <button onClick={handleRefresh} disabled={loading} className="p-2 hover:bg-white/5 rounded-lg text-zinc-500 hover:text-white transition-colors" title="Refresh Live Price">
            ...
            {!isLive && !hasError && (
                <div className="flex gap-2 mt-2 pt-3 border-t border-white/5 relative z-10">
                     <button 
                        ...
                        className="flex-1 py-2 rounded-lg bg-lux-gold/10 hover:bg-lux-gold/20 text-xs font-bold text-lux-gold border border-lux-gold/20 transition-colors flex items-center justify-center gap-2"
                    >
