...
export const OnboardingTutorial: React.FC<{ user: User; onComplete: () => void }> = ({ user, onComplete }) => {
  ...
  return (
    <div className="fixed inset-0 z-[200] overflow-hidden">
      ...
      >
        <Card className={`pointer-events-auto w-full max-w-sm p-6 border-lux-gold shadow-2xl relative animate-in fade-in zoom-in-95 duration-300`}>
          {coords && coords.top + coords.height + 20 > window.innerHeight - 200 ? (
             <div className="absolute left-1/2 -translate-x-1/2 -bottom-2 w-4 h-4 bg-[#121318] border-b border-r border-lux-gold/30 rotate-45 transform"></div>
          ) : coords ? (
             <div className="absolute left-1/2 -translate-x-1/2 -top-2 w-4 h-4 bg-[#121318] border-t border-l border-lux-gold/30 rotate-45 transform"></div>
          ) : null}
